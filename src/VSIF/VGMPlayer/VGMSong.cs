...
namespace zanac.VGMPlayer
{
    public class VGMSong : SongBase
    {
        ...
        /// <summary>
        /// VGM compressed data block (type 0x40..0x7E) の展開処理
        /// 参考: https://vgmrips.net/wiki/VGM_Specification  Data blocks 40..7E
        /// </summary>
        /// <param name="compressedData">圧縮データ (0x0A bytes) のヘッダ情報を含む</param>
        /// <param name="compressionType">tt: 0=bit packing, 1=DPCM</param>
        /// <param name="subType">st: bit packing の場合 (0=copy,1=shift,2=table) / DPCM の場合 0 固定</param>
        /// <param name="bitsDecompressed">bd: 展開後のビット数</param>
        /// <param name="bitsCompressed">bc: 圧縮後のビット数</param>
        /// <param name="addend">aa: bit packing の場合の初期値 (table 使用時は無視)</param>
        /// <param name="uncompressedSize">展開後のデータサイズ</param>
        /// <returns>展開後のデータ</returns>
            ...
            byte bitsDecompressed, byte bitsCompressed, ushort addend, uint uncompressedSize)
        {
            ...
            // vgmplay と同じビット読み取りアルゴリズム:
            //   各バイト内は上位ビット(MSB)から読む。
            //   8ビット単位のチャンクを出力値の下位ビットから順に積み上げる。
            //   cf. DecompressDataBlk in vgmplay/VGMPlay.c
            int inShift = 0;    // 現在のバイト内で消費済みのビット数 (0..7)
            int srcIndex = 0;
            ...
            if (compressionType == 0) // bit packing
            {
                for (int i = 0; i < valueCount; i++)
                {
                    ...
                    // 展開後のデータを bytesPerValue バイト単位で追加
